                ...
                for (int obj_j = 0; obj_j < current_object->shape[0].size(); obj_j++) {
                    ...
                    if ((grid_row == i && grid_col == j || grid_row == i && grid_col == j + 1) 
                        ...
                        if (current_object->object_type == ASTEROID) {
                            ...
                            CelestialObject::delete_rotations(current_object);
    ...
    if (!collision) {
        for (int i = 0; i < current_object->shape.size(); i++) {
            for (int j = 0; j < current_object->shape[0].size(); j++) {
                int grid_row = current_object->starting_row + i;
                int grid_col = start_col + j;
                if (current_object->shape[i][j] == 1 && grid_row < new_grid.size() && grid_col < new_grid[0].size() && grid_col >= 0) {
        ...
        if (player->lives > 0) {
            ...
            CelestialObject* to_delete = current_object;
            current_object = current_object->next_celestial_object;
            CelestialObject::delete_rotations(to_delete);
            delete to_delete;
...
AsteroidDash::~AsteroidDash() {
    // TODO: Your code here
    // Delete all celestial objects and their rotations
    CelestialObject* temp = celestial_objects_list_head;
    while (temp != nullptr) {
        CelestialObject* next = temp->next_celestial_object;
